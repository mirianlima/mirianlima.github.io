...
con = duckdb.connect(database="duckdb_test.db", read_only=True)
...
def get_schema_description(con, table_name):
    ...
    return (
        f"The table name {table_name} contains the following columns: {schema_result}"
    )
...
if st.button("Get Data"):
    if user_query:
        ...
        schema_description = get_schema_description(con, "c_dem")
...
def update_database_with_csv(con, uploaded_file):
    if uploaded_file is not None:
        ...
        con.execute(
            f"CREATE OR REPLACE TABLE daily_stats AS SELECT * FROM '{uploaded_file}'"
        )
